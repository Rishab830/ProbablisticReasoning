##
# @file
# @brief Optic disc and cup segmentation using Naive Bayes classifier.
#
# This module implements a memory-efficient pipeline for retinal image segmentation using
# a Gaussian Naive Bayes classifier with spatial and color features to segment optic disc,
# cup, and background regions from fundus images.
#
...
# the specified maximum size while maintaining aspect ratio using area interpolation.
#
...
# nearest neighbor interpolation to avoid creating intermediate values.
#
...
#
# Converts grayscale mask values to exactly three discrete labels based on
# intensity thresholds. Handles cases where resizing creates intermediate values.
# - Values 0-63 map to 0 (disc)
# - Values 64-191 map to 128 (cup)
# - Values 192-255 map to 255 (background)
#
...
#
# Extracts pixel-wise features including RGB and optionally HSV color information
# along with normalized or absolute spatial coordinates for training or prediction.
#
...
#         8 (RGB + HSV + x + y) if use_both_colorspaces=True, or
#         5 (RGB + x + y) if use_both_colorspaces=False.
...
##
# @brief Train Naive Bayes classifier using batch processing and incremental learning.
#
...
# Uses partial_fit for online learning across batches.
#
...
#
# @return Tuple of (trained_classifier, fitted_scaler) where:
#         - trained_classifier is a GaussianNB object trained on all batches
#         - fitted_scaler is a StandardScaler for feature normalization
...
##
# @brief Predict segmentation mask for a test image.
#
# Extracts features from the test image, standardizes them using the fitted scaler,
# and predicts pixel-wise class labels using the trained Naive Bayes classifier.
#
...
#
# @return Predicted segmentation mask of shape (height, width) with values in {0, 128, 255}.
...
# and ground truth segmentation for a single class label. The Dice coefficient
# is calculated as: 2 * |X ∩ Y| / (|X| + |Y|)
#
# @param pred Predicted segmentation mask.
# @param gt Ground truth segmentation mask.
# @param label Class label value to evaluate (0, 128, or 255).
#
# @return Dice coefficient value in range [0, 1] where 1 indicates perfect overlap.
...
##
# @brief Calculate Intersection over Union (IoU) for a specific segmentation class.
#
# Computes the IoU (Jaccard index) between predicted and ground truth segmentation
# for a single class label. IoU is calculated as: |X ∩ Y| / |X ∪ Y|
#
# @param pred Predicted segmentation mask.
# @param gt Ground truth segmentation mask.
# @param label Class label value to evaluate (0, 128, or 255).
#
# @return IoU value in range [0, 1] where 1 indicates perfect overlap.
...
#
# Computes comprehensive evaluation metrics including overall pixel accuracy,
# per-class Dice coefficients and IoU scores for all three classes
# (disc, cup, background), and their mean values.
#
# @param pred_mask Predicted segmentation mask.
# @param gt_mask Ground truth segmentation mask.
#
# @return Dictionary containing the following metrics:
#         - 'accuracy': Overall pixel accuracy
#         - 'dice_disc': Dice coefficient for disc class (0)
#         - 'dice_cup': Dice coefficient for cup class (128)
#         - 'dice_background': Dice coefficient for background class (255)
#         - 'dice_mean': Mean Dice across all classes
#         - 'iou_disc': IoU for disc class (0)
#         - 'iou_cup': IoU for cup class (128)
#         - 'iou_background': IoU for background class (255)
#         - 'iou_mean': Mean IoU across all classes
...
##
# @brief Save evaluation metrics as a formatted table image.
#
# Creates and saves a styled table displaying all evaluation metrics including
# pixel accuracy, Dice coefficients, and IoU scores for a single test image.
# The table includes color-coded rows and highlighted mean scores.
#
# @param metrics_dict Dictionary containing evaluation metrics from evaluate_segmentation().
# @param image_name Name of the image being evaluated (used in title).
# @param save_path Full file path where the metrics table image will be saved.
#
# @return None. Saves the metrics table as a PNG image to the specified path.
...
##
# @brief Save overall summary metrics across all test images as a table image.
#
# Computes mean and standard deviation for all metrics across multiple test images
# and saves a formatted table displaying the aggregated statistics.
#
# @param all_metrics List of metrics dictionaries from all test images.
#                    Each dictionary should have the same keys as returned by evaluate_segmentation().
# @param save_path Full file path where the summary metrics table image will be saved.
#
# @return None. Saves the summary metrics table as a PNG image. Returns early if all_metrics is empty.
...
##
# @brief Visualize segmentation results with side-by-side comparison.
#
# Creates a visualization comparing the original image, predicted segmentation mask,
# and optionally the ground truth mask. Saves the visualization as an image file.
#
# @param original Original fundus image (BGR format).
# @param predicted_mask Predicted segmentation mask with values {0, 128, 255}.
# @param gt_mask Ground truth mask (optional). If provided, creates 3-panel visualization.
#                If None, creates 2-panel visualization. Default is None.
# @param save_path File path to save the visualization image. If None, the figure is
#                  only closed without saving. Default is None.
#
# @return None. Either saves the visualization to disk or closes the figure.
...
# - Configuration of data directories and processing parameters
# - Loading and preprocessing training data with memory-efficient batching
# - Training Gaussian Naive Bayes classifier with incremental learning
# - Testing on validation images with ground truth evaluation
# - Computing comprehensive metrics (accuracy, Dice, IoU)
# - Saving visualizations and metric tables for individual images
# - Generating overall summary statistics across all test images
#
# The pipeline uses memory optimization techniques including image resizing,
# batch processing, and explicit garbage collection to handle large datasets.
#
# @return None. All outputs (predictions, metrics, visualizations) are saved to OUTPUT_DIR.
def main():
...
#
# Executes the main() function when the script is run directly as a standalone program.

##
# @file
# @brief Optic disc and cup segmentation using Naive Bayes classifier with Dense CRF refinement and normalized features.
#
# This module implements a memory-efficient pipeline for retinal image segmentation that combines
# Gaussian Naive Bayes classification with normalized color features and Dense Conditional Random 
# Field (CRF) post-processing to segment optic disc, cup, and background regions from fundus images.
# Includes comprehensive metrics comparison with mean and standard deviation reporting.
#
...
##
# @brief Extract color and spatial features from fundus image with optional normalization.
#
# Extracts pixel-wise features including RGB and optionally HSV color information
# along with normalized or absolute spatial coordinates. Supports feature normalization
# to [0,1] range for improved numerical stability.
#
...
# @param normalize_colors If True, normalize color values to [0,1] range. Default is True.
#
...
# Uses partial_fit for online learning across batches with normalized features.
#
...
# using the trained Naive Bayes classifier with normalized features.
#
...
#
# Generates segmentation predictions using Naive Bayes classifier with normalized
# features and optionally applies Dense CRF post-processing for refinement.
#
# @param clf Trained GaussianNB classifier.
# @param scaler Fitted StandardScaler for feature normalization.
...
# and per-class Dice coefficients for all three classes (disc, cup, background).
#
...
# Naive Bayes and CRF-refined predictions for a single test image, with
...
##
# @brief Save overall summary metrics with mean and standard deviation statistics.
#
# Computes and saves a comprehensive table showing mean and standard deviation
# for all metrics across multiple test images, comparing Naive Bayes and CRF
# performance with improvement indicators.
#
...
#
# @return None. Saves image to disk or returns early if metrics_crf_list is empty.
...
#
# Creates a side-by-side visualization comparing original image, ground truth (if available),
...
##
# @brief Memory-efficient main execution pipeline with normalized features and CRF refinement.
#
...
# - Training Gaussian Naive Bayes classifier with normalized color features
# - Testing on validation images with ground truth evaluation
# - Applying Dense CRF post-processing for segmentation refinement
# - Computing comprehensive metrics with mean and standard deviation statistics
# - Saving individual and summary visualizations and metric tables
#
# The pipeline uses memory optimization techniques including image resizing,
# batch processing, normalized features, and explicit garbage collection.
#
